...
const {U,S,F,B,isObj,def} = core.types;

// A list of valid load modes.
...
function validateLoadSetting(prop, value, reportInvalid, reportUnknown)
{
  ...
  { // Call the validator, which will return `true`, `false`, or in some cases, a new valid value.
    const isValid = LUM_LOAD_VALID[prop].call(load, value)
...
function load()
{
  return load.globalContext.loadFrom(...arguments);
}
  ...
  for (let a=0; a < arguments.length; a++)
  {
    ...
    { // Okay, time to pass it on to a loader method.
      ...
      { // Determine the loader to use based on the extension.
        ...
        {
          loaderFunc = load.data;
        }
      } 
      else
      { // Use a directly specified loading mode.
        loaderFunc = load[this.mode];
      }
...
function setupSettings (settings)
{
  ...
  def(settings, 'loadFrom', loadFromSetting);

  // Load resources using these settings.
  def(settings, 'loadUsing', loadUsingSetting);
  ...
  def(settings, 'populate', function(obj, ...props)
  {
...
// Global load defaults. Should only be changed via `load.set()`
def(load, 'globalContext', setupSettings({}), false);

/**
 * Update global settings for the `load()` method.
 */
def(load, 'set', function (opts, validate)
{
  return load.globalContext.set(opts, validate);
});

/**
 * Reset the global settings for the `load()` method.
 */
def(load, 'reset', function()
{
  load.set(LUM_LOAD_DEFAULTS, false);
});

load.reset(); // Set the defaults now.

function getLoaderOpts(caller, args, objProps, objAttrs)
{
  const DEFS = LUM_LOAD_DEFAULT_OPTS;
  const GC = load.globalContext;
  let opts;
...
{ 
  ...
  { // Node works differently.
    if (load.$required === undefined)
    {
      load.$required = {};
    }
    const lib = load.$required[opts.url] = require(url);
    if (typeof opts.func  === F)
    {
...
/**
 * A convenience method for loading v4-style LumJS modules.
 * This isn't an actual loader mode.
 */
def(load, 'modules', function(...modules)
{
